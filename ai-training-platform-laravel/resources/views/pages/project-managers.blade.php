    ...
    <x-section background="hero" size="md">
        <x-container>
            <div class="text-center mb-spacing-xl">
                <div class="inline-flex items-center gap-spacing-sm bg-accent-project-managers/10 text-accent-project-managers px-3 py-1.5 rounded-full text-small font-medium mb-spacing-md border border-accent-project-managers/20">
                    <div class="w-1.5 h-1.5 bg-accent-project-managers rounded-full animate-pulse"></div>
                    Project Manager Track
                </div>
                <x-heading level="1" class="mb-spacing-md">AI Project Management</x-heading>
        ...
        <x-container>
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-spacing-lg">
                @foreach($modules as $module)
                    <x-holo-card role="project-managers" class="h-full flex flex-col hover:-translate-y-1 transition-transform">
                        <div class="flex items-center spacing-x-md mb-spacing-md">
                            <div class="w-12 h-12 bg-gradient-to-br from-accent-project-managers to-accent-project-managers/70 rounded-lg flex items-center justify-center">
                                <span class="text-lg font-bold text-white">{{ $module['number'] }}</span>
                            </div>
                            <div>
                                <h2 class="text-heading-3">{{ $module['title'] }}</h2>
                                <div class="text-small bg-accent-project-managers/10 px-2 py-1 rounded">{{ $module['badge'] }}</div>
                            </div>
                        </div>
                        ...
                        <div class="spacing-y-sm mb-spacing-lg">
                            @foreach($module['features'] as $feature)
                                <div class="flex items-center spacing-x-sm text-small">
                                    <span class="w-2 h-2 bg-accent-project-managers rounded-full"></span>
                                    {{ $feature }}
                                </div>
                            @endforeach
                        </div>
                        ...
                        <x-progress-bar value="{{ $module['progress'] }}" color="project-managers" show-label="false" class="mb-spacing-md" />
                        <x-button href="/project-managers/module-{{ $module['number'] }}" variant="default" size="lg" class="w-full">
