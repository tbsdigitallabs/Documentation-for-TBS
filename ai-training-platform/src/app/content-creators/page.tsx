...
export default function ContentCreatorsPage() {
  ...
  const isUserClass = userClass === "Storyteller";
          ...
          <div className="text-center mb-16">
            ...
            {!isUserClass && userClass && (
              <p className="text-content-tertiary text-sm mb-4">
                You're exploring the Storyteller class. Your primary class is {getClassDisplayName(userClass)}.
              </p>
            )}
      ...
      <Section className="bg-gradient-surface py-16" size="lg">
        <Container size="xl">
          <h2 className="text-2xl font-heading font-bold text-content-primary mb-6">
            {isUserClass ? "Your Adventures" : "Storyteller Adventures"}
          </h2>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 items-stretch mb-12">
            {modules.map((module) => (
              <HoloCard key={module.slug} role="content-creators" className="flex flex-col h-full">
                <div className="mb-6">
                  <span className="text-xs font-mono text-accent-content-creators uppercase tracking-wider px-2 py-1 rounded-md bg-accent-content-creators-5 border border-accent-content-creators-10 inline-block">
                ...
                <Link href={`/content-creators/${module.slug}`} className="mt-auto">
                  <Button className="w-full bg-accent-content-creators text-white hover:bg-accent-content-creators-90 py-3 text-base">
                    Begin Adventure
                  </Button>
                </Link>
              </HoloCard>
