...
export default function DesignersPage() {
  ...
  const isUserClass = userClass === "Bard";
          ...
          <div className="text-center mb-16">
            ...
            {!isUserClass && userClass && (
              <p className="text-content-tertiary text-sm mb-4">
                You're exploring the Bard class. Your primary class is {getClassDisplayName(userClass)}.
              </p>
            )}
      ...
      <Section className="bg-gradient-surface py-16" size="lg">
        <Container size="xl">
          <h2 className="text-2xl font-heading font-bold text-content-primary mb-6">
            {isUserClass ? "Your Adventures" : "Bard Adventures"}
          </h2>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 items-stretch mb-12">
            {modules.map((module) => (
              <HoloCard key={module.slug} role="designers" className="flex flex-col h-full">
                <div className="mb-6">
                  <span className="text-xs font-mono text-accent-designers uppercase tracking-wider px-2 py-1 rounded-md bg-accent-designers-5 border border-accent-designers-10 inline-block">
                ...
                <Link href={`/designers/${module.slug}`} className="mt-auto">
                  <Button className="w-full bg-accent-designers text-white hover:bg-accent-designers-90 py-3 text-base">
                    Begin Adventure
                  </Button>
                </Link>
              </HoloCard>
